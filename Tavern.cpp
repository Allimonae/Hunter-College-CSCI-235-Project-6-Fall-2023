...
Georgina Woo
June 20 2023
Modified for Project 6 by Allison Lee on November 25, 2023
Taven.cpp declares the Tavern class along with its private and public members
*/
#include "Tavern.hpp"

/** Default Constructor **/
Tavern::Tavern() : ArrayBag<Character*>(), level_sum_{0}, num_enemies_{0}, main_character_{nullptr}
{
...
*/
bool Tavern::levelIsLess(Character* const character1, Character* const character2) const{
...
*/
bool Tavern::levelIsGreater(Character* const character1, Character* const character2) const{
  return character1->getLevel() > character2->getLevel();
}
...
*/
bool Tavern::vitalityIsLess(Character* const character1, Character* const character2) const{
...
*/
bool Tavern::vitalityIsGreater(Character* const character1, Character* const character2) const{
  return character1->getVitality() > character2->getVitality();
}
...
void Tavern::createCombatQueue(const std::string& filter){
  ...
  if(filter == "LVLASC"){
    std::sort(characters.begin(), characters.end(), 
      [](Character* const character1, Character* const character2){
        return character1->getLevel() < character2->getLevel();
      });
  }
...
void Tavern::actionSelection(){
  ...
  while(!getMainCharacter()->getActionQueue().empty()){
    getMainCharacter()->getActionQueue().pop();
  }
  ...
  for (int i = 0; i < 3; i++){
    ...
    getMainCharacter()->addAction(input_action);
  }
}
...
void Tavern::turnResolution(){
  ...
  std::stack<Buff> buffs = getMainCharacter()->getBuffStack();
  if(!buffs.empty()){
    std::string buff_name = buffs.top().name_;
    getMainCharacter()->useBuff();
    printActionResult(buff_name);
  }
  while(!getMainCharacter()->getActionQueue().empty()){
    int curr_action = getMainCharacter()->getActionQueue().front();
    getMainCharacter()->popAction();
    if (curr_action == 1){
      Buff heal = {"Heal", 3};
      getMainCharacter()->heal();
      heal.turns_ --;
      getMainCharacter()->addBuff(heal);
      printActionResult("Heal");
    }
    else if (curr_action == 2){
      Buff mend = {"MendMetal", 2};
      getMainCharacter()->mendMetal();
      mend.turns_ --;
      getMainCharacter()->addBuff(mend);
      printActionResult("MendMetal");
    }
    else if (curr_action == 3){
      getMainCharacter()->strike(getTarget());
      printActionResult("Strike");
      targetDefeated();
      std::cout << std::endl;
    }
    else if (curr_action == 4){
      getMainCharacter()->throwTomato(getTarget());
      printActionResult("ThrowTomato");
      targetDefeated();
      std::cout << std::endl;
    }
    if (combat_queue_.empty()){
...
/**
  helper function for turnResolution()
  @post:  Print out the results of the action as described below:
          After applying each action, print out the results of the action as follows:
          [MAINCHARACTER NAME] used [ACTION NAME]!
          \n(YOU) [MAINCHARACTER NAME]: LEVEL [MAINCHARACTER LEVEL] [MAINCHARACTER RACE]. \nVITALITY: [MAINCHARACTER VITALITY] \nARMOR: [MAINCHARACTER ARMOR]\n
          \n(ENEMY) [ENEMY NAME]: LEVEL [ENEMY LEVEL] [ENEMY RACE]. \nVITALITY: [ENEMY VITALITY] \nARMOR: [ENEMY ARMOR]\n 
*/
void Tavern::printActionResult(const std::string& action_name) const{
...
/**
  @post:Whenever an enemy's vitality becomes <=0 after an attack, also print out 
        "[CHARACTER NAME] DEFEATED\n" 
        and remove the enemy from the combat queue AND let them exit the tavern. 
*/
void Tavern::targetDefeated(){
...
/**
  ...
  @post   : Do all of the following:
          ...
          : \n(YOU) [MAINCHARACTER NAME]: LEVEL [MAINCHARACTER LEVEL] [MAINCHARACTER RACE]. \nVITALITY: [MAINCHARACTER VITALITY] \nARMOR: [MAINCHARACTER ARMOR]\n
*/
void Tavern::enemyTurn(Character* const enemy){
  std::cout << "ENEMY TURN\n";
  std::stack<Buff> buffs = getTarget()->getBuffStack();
  if(!buffs.empty()){
    std::string buff_name = buffs.top().name_;
    getTarget()->useBuff();
    printEnemyActionResult(buff_name);
  }
  int curr_action = (rand() % 4) + 1;
  if (curr_action == 1){
    Buff heal = {"Heal", 3};
    getTarget()->heal();
    heal.turns_ --;
    getTarget()->addBuff(heal);
    printEnemyActionResult("Heal");
  }
  else if (curr_action == 2){
    Buff mend = {"MendMetal", 2};
    getTarget()->mendMetal();
    mend.turns_ --;
    getTarget()->addBuff(mend);
    printEnemyActionResult("MendMetal");
  }
  else if (curr_action == 3){
    getTarget()->strike(getMainCharacter());
    printEnemyActionResult("Strike");
  }
  else if (curr_action == 4){
    getTarget()->throwTomato(getMainCharacter());
    printEnemyActionResult("ThrowTomato");
  }
  std::cout << "END OF ENEMY TURN\n\n";
}

/**
  helper function for enemyTurn()
  @param: a string reference to an action name
  @post: After applying each action, print out the results of the action as follows:
          [ENEMY NAME] used [ACTION NAME]!
          \n(ENEMY) [ENEMY NAME]: LEVEL [ENEMY LEVEL] [ENEMY RACE]. \nVITALITY: [ENEMY VITALITY] \nARMOR: [ENEMY ARMOR]\n 
          \n(YOU) [MAINCHARACTER NAME]: LEVEL [MAINCHARACTER LEVEL] [MAINCHARACTER RACE]. \nVITALITY: [MAINCHARACTER VITALITY] \nARMOR: [MAINCHARACTER ARMOR]\n
*/
void Tavern::printEnemyActionResult(const std::string& action_name) const{
  std::cout << getTarget()->getName() << " used " << action_name << "!\n(ENEMY) " <<
    getTarget()->getName() << ": LEVEL " << getTarget()->getLevel() << " " <<
    getTarget()->getRace() << ". \nVITALITY: " << getTarget()->getVitality() <<
