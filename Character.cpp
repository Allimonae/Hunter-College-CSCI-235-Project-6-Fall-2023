...
*/
void Character::strike(Character* const target){
    if (target->getArmor() >= 2){
        target->setArmor(target->getArmor() - 2);
    }
    else if (target->getArmor() < 2){
        int temp_armor = target->getArmor();
        target->setArmor(0);
        int vitality = target->getVitality() - 2 + temp_armor;
        if (vitality < 0){vitality = 0;}
        target->setVitality(vitality);
    }
}
...
*/
void Character::throwTomato(Character* const target){
...
/**
    @return: the value stored in action_queue_
*/
std::queue<int> Character::getActionQueue() const{
    return action_queue_;
}

/**
    @param: an integer to go into action queue
    @post: add integer to action_queue_
*/
...
/**
    @post: pop integer from action_queue_
*/
...
/**
    @param: a stack of Buffs
    @post: sets the buff stack private member to the value of the parameter\
*/
void Character::setBuffStack(std::stack<Buff> buffs){
    buff_stack_ = buffs;
}

/**
    @return: the value stored in buff_stack_
*/
std::stack<Buff> Character::getBuffStack() const{
    return buff_stack_;
}

/**
    @param: a buff to go into buff stack
    @post: add buff to buff_stack_
*/
void Character::addBuff(Buff add_buff){
    buff_stack_.push(add_buff);
}

/**
    helper function for turnResolution()
    @post: If the Buff stack is not empty, apply the buff once and decrement the turns.
